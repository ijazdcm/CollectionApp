...
class Agents
{
    ...
    public function get_all_agent()
    {
        ...
        $sql = "SELECT `AGENT_ID`,`AGENT_NAME`,`AGENT_PH_NO`,`AGENT_LOCATION`,`PLACE_NAME` FROM " . $this->table_name . "," . $this->table_place . " WHERE `AGENT_DL_STATUS`=1 AND `AGENT_LOCATION`=" . $this->table_place . ".PLACE_ID ORDER BY agent_master.AGENT_ID ASC";
        $stmt = $this->conn->prepare($sql);
        try {
            $stmt->execute();
