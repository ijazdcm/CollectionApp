...
class Customer
{
    ...
    {
        $sql = "INSERT INTO " . $this->table_name . "(`USER_ID`,`CUS_NAME`, `CUS_SUR_NAME`, `CUS_PM_PH_NO`, `CUS_SE_PH_NO`, `CUS_PLACE_ID`, `CUS_REF_BY`, `CUS_PLAN_ID`,`CUS_COM_ONE`,`CUS_SCHEME_ID`)
        VALUES (
            '" . $this->user_id . "',
            '" . $this->cus_name . "',
            '" . $this->cus_sur_name . "',
    ...
    public function super_count_by_plan()
    {
        ...
        for ($i = 0; $i < $total_plan_count; $i++) {
            ...
            $sql = "SELECT COUNT(`CUS_PLAN_ID`) as total FROM `" . $this->table_name . "` WHERE `CUS_PLAN_ID`=" . $all_plans_fetched[$i]['PL_ID'] . " AND `CUS_DL_STATUS` != 0";
            $stmt = $this->conn->prepare($sql);
            $stmt->execute();
    ...
    public function super_count_by_place()
    {
        ...
        for ($i = 0; $i < $total_place_count; $i++) {
            ...
            $sql = "SELECT COUNT(`CUS_PLACE_ID`) as total FROM `" . $this->table_name . "` WHERE `CUS_PLACE_ID`=" . $all_places_fetched[$i]['PLACE_ID'] . " AND `CUS_DL_STATUS` != 0";
            $stmt = $this->conn->prepare($sql);
            $stmt->execute();
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    public function super_count_by_agent()
    {
        ...
        for ($i = 0; $i < $total_agent_count; $i++) {
            ...
            $sql = "SELECT COUNT(`CUS_REF_BY`) as total FROM `" . $this->table_name . "` WHERE `CUS_REF_BY`=" . $all_agent_fetched[$i]['AGENT_ID'] . " AND `CUS_DL_STATUS` != 0";
            $stmt = $this->conn->prepare($sql);
            $stmt->execute();
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
